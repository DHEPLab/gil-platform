...
# ---------------------------------------------------------------------------------------------------------------
# generate_config_single.sh
#
# A Shell script to generate a configuration file for assigning cases to a single user (by email) in a database.
# This script connects to a PostgreSQL database, retrieves a random sample of cases for the given user,
# flags a random subset as “risk assessments,” and writes out a CSV in the lab’s OMOP format.
#
...
# 7. Number of AI (“risk”) cases per user
# 8. Output CSV file path (default: config.csv)
#
# To run:
...
# Informational log
echo "[1/1] Processing user: $USER_EMAIL"

# Generate and append rows for this single user
psql -h "$DB_HOST" \
     -p "$DB_PORT" \
     -U "$DB_USER" \
     -d "$DB_NAME" \
